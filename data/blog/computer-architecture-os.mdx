...
- Memory is temporary storage that holds command and resource for the currently running program.
- For a program to run, it must be stored in memory.
- However, memory is expensive to store, so only some of the program's code and resources are loaded.
