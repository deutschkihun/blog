...
## 6. Request header / body & Response header / body

chrome 기준으로 request header 와 response header 는 headers tab 에서 확인 가능. header 에는 각각 request 와 response 에 대한 정보들이 들어가 있습니다.

chrome 기준으로 request body 는 바로 request payload 와 동일합니다.

![main](/static/network/request-body.png)

chrome 기준으로 response body 는 response tab 에서 확인 가능합니다.

![main](/static/network/response-body.png)
